...
public class Define {
    ...
    /*El método cant_var de esta clase se encarga de recibir la cadena asi tal cual
    a secas llena de comas, lo que se hace es solo leerla y obtener la cantidad de valores
    resultantes (subcadenas) al quitarle las comas*/
    ...
    public String[] origen_a (String[]pnl,int oG){
        int tam;
        String calcuA[] = null;
        //System.out.println("El origen es la posicion "+oG+" con el valor "+pnl[oG]);
        tam = (pnl.length-(pnl.length-oG));
        //System.out.println("El nuevo tamaño es de :D "+tam+ " valorsitos.");
        calcuA = new String[tam];
        for(int j=0;j<tam;j++){
            calcuA[j] = pnl[oG-1];
            //System.out.println("Ahora se encuentra el valor "+pnl[i]+" en la posicion "+j+" del nuevo arreglo :D");
          oG--;
    ...
    public String[] acomodador(String[]total1a){
       int size = total1a.length;
       int pos = size-1;
       String []total1;
       total1 = new String[size];
